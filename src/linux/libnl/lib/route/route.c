...
struct nl_msg *rtnl_route_build_request(struct rtnl_route *rt, int flags)
{
	struct nl_msg *msg;
	struct rtmsg r;
	...
	msg = nlmsg_build_simple(RTM_NEWROUTE, flags);
	if (!msg)
		return NULL;

	r.rtm_family = rt->rt_family;
	r.rtm_table = RT_TABLE_MAIN;
	r.rtm_scope = RT_SCOPE_UNIVERSE;
	r.rtm_protocol = RTPROT_BOOT;
	r.rtm_type = RTN_UNICAST;

	if (rt->rt_mask & ROUTE_ATTR_DST)
		r.rtm_dst_len = nl_addr_get_prefixlen(rt->rt_dst);

	if (rt->rt_mask & ROUTE_ATTR_SRC)
		r.rtm_src_len = nl_addr_get_prefixlen(rt->rt_src);

	if (rt->rt_mask & ROUTE_ATTR_TOS)
		r.rtm_tos = rt->rt_tos;

	if (rt->rt_mask & ROUTE_ATTR_TABLE)
		r.rtm_table = rt->rt_table;

	if (rt->rt_mask & ROUTE_ATTR_SCOPE)
		r.rtm_scope = rt->rt_scope;

	if (rt->rt_mask & ROUTE_ATTR_TYPE)
		r.rtm_type = rt->rt_type;

	if (nlmsg_append(msg, &r, sizeof(r), 1) < 0)
		goto nla_put_failure;

	if (rt->rt_mask & ROUTE_ATTR_DST)
		NLA_PUT_ADDR(msg, RTA_DST, rt->rt_dst);

	if (rt->rt_mask & ROUTE_ATTR_SRC)
		NLA_PUT_ADDR(msg, RTA_SRC, rt->rt_src);

	if (rt->rt_mask & ROUTE_ATTR_GATEWAY)
		NLA_PUT_ADDR(msg, RTA_GATEWAY, rt->rt_gateway);

	if (rt->rt_mask & ROUTE_ATTR_PREF_SRC)
		NLA_PUT_ADDR(msg, RTA_PREFSRC, rt->rt_pref_src);

	if (rt->rt_mask & ROUTE_ATTR_REALMS)
		NLA_PUT_U32(msg, RTA_FLOW, rt->rt_realms);

	if (rt->rt_mask & ROUTE_ATTR_PRIO)
		NLA_PUT_U32(msg, RTA_PRIORITY, rt->rt_prio);

	if (rt->rt_mask & ROUTE_ATTR_OIF)
		NLA_PUT_U32(msg, RTA_OIF, rt->rt_oif);
...
struct rtnl_route *rtnl_route_alloc(void)
{
	...
	route = (struct rtnl_route *) nl_object_alloc_from_ops(&rtnl_route_ops);
	if (route == NULL)
		return NULL;

	nl_init_list_head(&route->rt_nexthops);
