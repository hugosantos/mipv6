...
struct rtnl_link *rtnl_link_update_cache_from_msg(struct nl_cache *cache,
						  struct nlmsghdr *n)
{
	...
	if (nlmsg_parse(n, sizeof(*ifi), tb, IFLA_MAX, link_policy) < 0)
		return NULL;
	...
	link = rtnl_link_get(cache, ifi->ifi_index);

	if (n->nlmsg_type == RTM_DELLINK) {
		if (link == NULL)
			return NULL;

		/* next put will free the link */
		nl_cache_delete(cache, (struct nl_object *)link);
	...
	if (link == NULL) {
		...
		nl_cache_add(cache, (struct nl_object *)link);
